...
struct FactorGraph{T <: Integer, V <: AbstractVariableType, F <: AbstractFactorType} <:
       AbstractGraph{T}
    graph::SimpleGraph{T}
    labels::BiDictMap{T}
...
function FactorGraph{T, V, F}(
    nv::Int = 100,
    nf::Int = 100,
) where {T <: Integer, V <: AbstractVariableType, F <: AbstractFactorType}
