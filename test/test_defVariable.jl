...
@testset "Testing @defVariable" begin
    ...
    @test getPointType(TestVarType2) ==
          ArrayPartition{Float64, Tuple{Vector{Float64}, Matrix{Float64}}}

    @test getPointIdentity(TestVarType1) == zeros(3)
    @test getPointIdentity(TestVarType2).x[1] ==
          ArrayPartition(zeros(3), diagm(ones(3))).x[1]
    @test getPointIdentity(TestVarType2).x[2] ==
          ArrayPartition(zeros(3), diagm(ones(3))).x[2]
